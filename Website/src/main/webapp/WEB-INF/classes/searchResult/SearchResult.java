	...
	public String formatResults() {
		...
		for (ResultEntry r : results){
			out += "<div class='result'>" 
						...
						+ "<br><sub><a href='"+r.url+"'>"+r.url+"</a></sub>"
						+ "<br><sub>Local(In: <i>"+r.localIn+"</i> Out: <i>"+r.localOut+"</i>) | Global(In: <i>"+r.globalIn+"</i> Out: <i>"+r.globalOut+"</i>)</sub>"
						+"<p class='summaryText'>"+lorem[i++]+"</p>"
					+ "</div>";
			if(i==lorem.length)
				i=0;
		}
